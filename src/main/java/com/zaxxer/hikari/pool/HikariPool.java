...
public final class HikariPool extends PoolBase implements HikariPoolMXBean, IBagStateListener
{
   ...
   /**
    * 并发安全的Bag 存储Connection  这里用PoolEntry 做了二次封装
    */
   ...
   /**
    * 定时执行任务 用于执行houseKeeping 任务   实现 MBean 动态配置  回收连接 现在连接
    */
   ...
   public HikariPool(final HikariConfig config)
   {
      ...
      // 该线程池负责 close connection操作
      this.closeConnectionExecutor = createThreadPoolExecutor(config.getMaximumPoolSize(), poolName + " connection closer", threadFactory, new ThreadPoolExecutor.CallerRunsPolicy());

      // 内存泄漏任务侦测工厂 ScheduleExecutorService
   ...
   void closeConnection(final PoolEntry poolEntry, final String closureReason)
   {
      if (connectionBag.remove(poolEntry)) {
         ...
         // 调用closeConnectionExecutor 执行关闭连接操作
   ...
   public String toString() {
      return "HikariPool{" +
         "poolState=" + poolState +
         ", aliveBypassWindowMs=" + aliveBypassWindowMs +
         ", housekeepingPeriodMs=" + housekeepingPeriodMs +
         ", poolEntryCreator=" + poolEntryCreator +
         ", postFillPoolEntryCreator=" + postFillPoolEntryCreator +
         ", addConnectionQueue=" + addConnectionQueue +
         ", addConnectionExecutor=" + addConnectionExecutor +
         ", closeConnectionExecutor=" + closeConnectionExecutor +
         ", connectionBag=" + connectionBag +
         ", leakTaskFactory=" + leakTaskFactory +
         ", suspendResumeLock=" + suspendResumeLock +
         ", houseKeepingExecutorService=" + houseKeepingExecutorService +
